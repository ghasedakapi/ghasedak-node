...
class Ghasedak extends Request {
	 ...
	 * @param {string} [apiBase=developers.ghasedak.io] - api address
	 * @param {string} [apiVersion=/api/v1] - api version
	 */
	constructor(apiKey, apiVersion = "/api/v1", apiBase = "developers.ghasedak.io") {
		super();
	...
	verification(opts) {
		...
		/** send voice call request */
		this.request("/sms/send/verification ", {
			receptor: this.__receptor,
			type: this.__type,
			template: this.__template,
			ip: opts.ip,
	...
	check_verification(opts) {
		...
		/** send voice call request */
		this.request("/sms/check/verification ", {
			receptor: this.__receptor,
			token: this.__token,
			ip: opts.ip
		});
